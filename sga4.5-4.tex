...
$\sF$-torseurs$)\to \h^1(X,\sF)$ de telle sorte que pour toute suite exacte 
$0\to \sF\xrightarrow\alpha\sG\xrightarrow\beta\sH\to 0$ et tout 
$h\in\h^0(X,\sH)$, le $\sF$-torseur $\beta^{-1}(h)\subset \sG$, sur lequel 
$\sF$ agit par $(f,x)\mapsto \alpha(f)+x$, soit de classe $\partial h$. 
...
Soit $\partial:\h^i(X\setminus D,\dG_m)\to \h_D^{i+1}(X,\dG_m)$ le morphisme 
...
faisceaux de cohomologie à support $\eR^p i^! \dmu_n$ sont nuls pour $p=0,1$, 
et $\eR^2 i^!\dmu_n = \underline{\dZ/n}$, engendré par 
$\cl_n(D)$. 
\end{proposition}

Il suffit de prouver que, pour $X$ strictement locale et $D$ défini par un 
paramètre régulier, on a $\h_D^p(X,\dmu_n) = 0$ pour $p=0,1$ et 
